...
var server = http.createServer(
  ...
  function(req, res) {
    ...
    for(var i=0; i<productList.length; i++) {
      ...
      res.write(productList[i].nom+" / "+productList[i].taille+" / "+productList[i].prix+" / <span id='qte"+i+"'>"+productList[i].quantite+"</span> / <span id='ajout"+i+"'>+</span> <br> ");

      res.write("<script> var amount"+i+" = document.getElementById('qte"+i+"');");
      res.write("console.log(amount"+i+");");
      res.write("var add"+i+" = document.getElementById('ajout"+i+"');");
      res.write("console.log(ajout"+i+");");
      res.write("add"+i+".addEventListener('click', function(){amount"+i+".textContent=parseInt(amount"+i+".textContent)+1; });");
      res.write("</script>");
